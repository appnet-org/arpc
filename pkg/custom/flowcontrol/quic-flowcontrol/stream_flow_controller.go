...
func (c *streamFlowController) GetWindowUpdate(now monotime.Time) protocol.ByteCount {
	...
	if c.receiveWindowSize > oldWindowSize {
		logging.Debug("Increasing receive flow control window for stream %d to %d", zap.Int64("stream_id", int64(c.streamID)), zap.Int64("window_size", int64(c.receiveWindowSize)))
