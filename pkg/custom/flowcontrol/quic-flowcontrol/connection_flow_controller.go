...
func (c *connectionFlowController) GetWindowUpdate(now monotime.Time) protocol.ByteCount {
	...
	if oldWindowSize < c.receiveWindowSize {
		logging.Debug("Increasing receive flow control window for the connection to %d kB", zap.Int64("window_size", int64(c.receiveWindowSize/(1<<10))))
	}
...
func (c *connectionFlowController) EnsureMinimumWindowSize(inc protocol.ByteCount, now monotime.Time) {
	...
	if delta := newSize - c.receiveWindowSize; delta > 0 && c.allowWindowIncrease(delta) {
		...
		logging.Debug("Increasing receive flow control window for the connection to %d, in response to stream flow control window increase", zap.Int64("window_size", int64(newSize)))
	}
	c.startNewAutoTuningEpoch(now)
}
