...
func generateUnmarshalStub(g *protogen.GeneratedFile, msg *protogen.Message) {
	...
	g.P("    var header byte")
	g.P("    if err := binary.Read(reader, binary.LittleEndian, &header); err != nil { return fmt.Errorf(\"failed to read header: %w\", err) }")
	g.P()
	g.P("    fieldOrder := make([]byte, ", len(msg.Fields), ")")
	g.P("    if _, err := reader.Read(fieldOrder); err != nil { return fmt.Errorf(\"failed to read field order: %w\", err) }")
	g.P()
	...
	g.P("        if err := binary.Read(reader, binary.LittleEndian, &fieldID); err != nil { return fmt.Errorf(\"failed to read field ID: %w\", err) }")
	g.P("        if err := binary.Read(reader, binary.LittleEndian, &off); err != nil { return fmt.Errorf(\"failed to read field offset: %w\", err) }")
	g.P("        if err := binary.Read(reader, binary.LittleEndian, &len); err != nil { return fmt.Errorf(\"failed to read field length: %w\", err) }")
		...
		if isVariableLength(field) {
			...
			if field.Desc.Kind() == protoreflect.MessageKind {
				...
				if field.Desc.IsList() {
					...
					g.P("                    if err := binary.Read(itemReader, binary.LittleEndian, &itemLen); err != nil { return fmt.Errorf(\"failed to read item length: %w\", err) }")
					g.P("                    itemBytes := make([]byte, itemLen)")
					g.P("                    if _, err := itemReader.Read(itemBytes); err != nil { return fmt.Errorf(\"failed to read item bytes: %w\", err) }")
			...
			g.P(fmt.Sprintf("                    if err := binary.Read(bytes.NewReader(dataRegion[dataStart:dataStart+%d]), binary.LittleEndian, &m.%s[i]); err != nil { return fmt.Errorf(\"failed to read repeated field element: %%w\", err) }", getFieldSize(field), goName))
			...
			g.P(fmt.Sprintf("            if err := binary.Read(bytes.NewReader(dataRegion[offset:offset+%d]), binary.LittleEndian, &m.%s); err != nil { return fmt.Errorf(\"failed to read fixed field: %%w\", err) }", getFieldSize(field), goName))
