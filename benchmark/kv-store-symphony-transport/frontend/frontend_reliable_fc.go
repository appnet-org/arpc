...
func generateDeterministicStringReliableFC(keyID string, length int) string {
...
var kvClientReliableFC kv.KVServiceClient
...
func handlerReliableFC(w http.ResponseWriter, r *http.Request) {
	...
	keyStr := generateDeterministicStringReliableFC(keyID+"-key", keySize)
	valueStr := generateDeterministicStringReliableFC(keyID+"-value", valueSize)
	...
	case "get":
		...
		resp, err := kvClientReliableFC.Get(context.Background(), req)
		if err != nil {
	...
	case "set":
		...
		resp, err := kvClientReliableFC.Set(context.Background(), req)
		if err != nil {
...
func main() {
	err := logging.Init(getLoggingConfigReliableFC())
	if err != nil {
	...
	kvClientReliableFC = kv.NewKVServiceClient(client)

	http.HandleFunc("/", handlerReliableFC)
	logging.Info("HTTP server listening (reliable + flow control mode)", zap.String("addr", ":8080"))
