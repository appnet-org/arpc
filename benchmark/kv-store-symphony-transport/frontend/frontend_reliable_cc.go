...
func generateDeterministicStringReliableCC(keyID string, length int) string {
...
var kvClientReliableCC kv.KVServiceClient
...
func handlerReliableCC(w http.ResponseWriter, r *http.Request) {
	...
	keyStr := generateDeterministicStringReliableCC(keyID+"-key", keySize)
	valueStr := generateDeterministicStringReliableCC(keyID+"-value", valueSize)
	...
	case "get":
		...
		resp, err := kvClientReliableCC.Get(context.Background(), req)
		if err != nil {
	...
	case "set":
		...
		resp, err := kvClientReliableCC.Set(context.Background(), req)
		if err != nil {
...
func main() {
	err := logging.Init(getLoggingConfigReliableCC())
	if err != nil {
	...
	kvClientReliableCC = kv.NewKVServiceClient(client)

	http.HandleFunc("/", handlerReliableCC)
	logging.Info("HTTP server listening (reliable + congestion control mode)", zap.String("addr", ":8080"))
