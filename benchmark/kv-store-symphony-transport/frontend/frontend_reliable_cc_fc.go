...
func generateDeterministicStringReliableCCFC(keyID string, length int) string {
...
var kvClientReliableCCFC kv.KVServiceClient
...
func handlerReliableCCFC(w http.ResponseWriter, r *http.Request) {
	...
	keyStr := generateDeterministicStringReliableCCFC(keyID+"-key", keySize)
	valueStr := generateDeterministicStringReliableCCFC(keyID+"-value", valueSize)
	...
	case "get":
		...
		resp, err := kvClientReliableCCFC.Get(context.Background(), req)
		if err != nil {
	...
	case "set":
		...
		resp, err := kvClientReliableCCFC.Set(context.Background(), req)
		if err != nil {
...
func main() {
	err := logging.Init(getLoggingConfigReliableCCFC())
	if err != nil {
	...
	kvClientReliableCCFC = kv.NewKVServiceClient(client)

	http.HandleFunc("/", handlerReliableCCFC)
