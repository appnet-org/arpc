...
func generateDeterministicStringReliable(keyID string, length int) string {
...
var kvClientReliable kv.KVServiceClient

// getLoggingConfig reads logging configuration from environment variables with defaults
func getLoggingConfigReliable() *logging.Config {
...
func handlerReliable(w http.ResponseWriter, r *http.Request) {
	...
	keyStr := generateDeterministicStringReliable(keyID+"-key", keySize)
	valueStr := generateDeterministicStringReliable(keyID+"-value", valueSize)
	...
	case "get":
		...
		resp, err := kvClientReliable.Get(context.Background(), req)
		if err != nil {
	...
	case "set":
		...
		resp, err := kvClientReliable.Set(context.Background(), req)
		if err != nil {
...
func main() {
	err := logging.Init(getLoggingConfigReliable())
	if err != nil {
	...
	clientHandler := reliable.NewReliableClientHandler(
		...
		// dummyTimer, // Use dummy timer instead of udpTransport.GetTimerManager()
	)
	...
	kvClientReliable = kv.NewKVServiceClient(client)

	http.HandleFunc("/", handlerReliable)
	logging.Info("HTTP server listening (reliable mode)", zap.String("addr", ":8080"))
