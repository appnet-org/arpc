...
func generateDeterministicStringFC(keyID string, length int) string {
...
var kvClientFC kv.KVServiceClient

// getLoggingConfig reads logging configuration from environment variables with defaults
func getLoggingConfigFC() *logging.Config {
...
func handlerFC(w http.ResponseWriter, r *http.Request) {
	...
	keyStr := generateDeterministicStringFC(keyID+"-key", keySize)
	valueStr := generateDeterministicStringFC(keyID+"-value", valueSize)
	...
	case "get":
		...
		resp, err := kvClientFC.Get(context.Background(), req)
		if err != nil {
	...
	case "set":
		...
		resp, err := kvClientFC.Set(context.Background(), req)
		if err != nil {
...
func main() {
	err := logging.Init(getLoggingConfigFC())
	if err != nil {
	...
	clientHandler := flowcontrol.NewFCClientHandler(
		udpTransport,
	...
	fcFeedbackChain := transport.NewHandlerChain("ClientFCFeedbackHandlerChain", clientHandler)
	udpTransport.RegisterHandlerChain(fcFeedbackPacketType.TypeID, fcFeedbackChain, transport.RoleClient)

	kvClientFC = kv.NewKVServiceClient(client)

	http.HandleFunc("/", handlerFC)
	logging.Info("HTTP server listening (flow control mode)", zap.String("addr", ":8080"))
