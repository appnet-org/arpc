...
func generateDeterministicStringCCFC(keyID string, length int) string {
...
var kvClientCCFC kv.KVServiceClient
...
func handlerCCFC(w http.ResponseWriter, r *http.Request) {
	...
	keyStr := generateDeterministicStringCCFC(keyID+"-key", keySize)
	valueStr := generateDeterministicStringCCFC(keyID+"-value", valueSize)
	...
	case "get":
		...
		resp, err := kvClientCCFC.Get(context.Background(), req)
		if err != nil {
	...
	case "set":
		...
		resp, err := kvClientCCFC.Set(context.Background(), req)
		if err != nil {
...
func main() {
	err := logging.Init(getLoggingConfigCCFC())
	if err != nil {
	...
	kvClientCCFC = kv.NewKVServiceClient(client)

	http.HandleFunc("/", handlerCCFC)
	logging.Info("HTTP server listening (congestion control + flow control mode)", zap.String("addr", ":8080"))
