...
type kvServerReliableCCFCEncryption struct {
	mu          sync.RWMutex
...
func NewKVServerReliableCCFCEncryption(maxSize int) *kvServerReliableCCFCEncryption {
	if maxSize <= 0 {
		maxSize = 1000 // Default max size
	}
	return &kvServerReliableCCFCEncryption{
...
func (s *kvServerReliableCCFCEncryption) Get(ctx context.Context, req *kv.GetRequest) (*kv.GetResponse, context.Context, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *kvServerReliableCCFCEncryption) Set(ctx context.Context, req *kv.SetRequest) (*kv.SetResponse, context.Context, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *kvServerReliableCCFCEncryption) moveToEnd(key string) {
...
func (s *kvServerReliableCCFCEncryption) evictLRU() {
...
func main() {
	...
	if err := logging.Init(getLoggingConfigReliableCCFCEncryption()); err != nil {
		panic(err)
	}
	...
	// Verify encryption is enabled (should have been enabled by NewServer with true parameter)
	logging.Info("KV server initialized",
		zap.String("serverType", "reliable-cc-fc-encryption"),
	...
	kvServer := NewKVServerReliableCCFCEncryption(maxSize)
	kv.RegisterKVServiceServer(server, kvServer)

	logging.Info("Reliable + congestion control + flow control KV server starting", zap.String("addr", ":11000"))
	server.Start()
}
