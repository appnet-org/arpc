...
func NewKVServerCCFC(maxSize int) *kvServerCCFC {
	if maxSize <= 0 {
...
func (s *kvServerCCFC) Get(ctx context.Context, req *kv.GetRequest) (*kv.GetResponse, context.Context, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *kvServerCCFC) Set(ctx context.Context, req *kv.SetRequest) (*kv.SetResponse, context.Context, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *kvServerCCFC) moveToEnd(key string) {
...
func main() {
	...
	if err := logging.Init(getLoggingConfigCCFC()); err != nil {
		panic(err)
	}
	...
	kvServer := NewKVServerCCFC(maxSize)
	kv.RegisterKVServiceServer(server, kvServer)

	logging.Info("Congestion control + flow control KV server starting", zap.String("addr", ":11000"))
	server.Start()
}
