...
func NewKVServerReliable(maxSize int) *kvServerReliable {
	if maxSize <= 0 {
		maxSize = 1000 // Default max size
	}
	return &kvServerReliable{
...
func (s *kvServerReliable) Get(ctx context.Context, req *kv.GetRequest) (*kv.GetResponse, context.Context, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *kvServerReliable) Set(ctx context.Context, req *kv.SetRequest) (*kv.SetResponse, context.Context, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *kvServerReliable) moveToEnd(key string) {
...
func (s *kvServerReliable) evictLRU() {
...
func main() {
	...
	if err := logging.Init(getLoggingConfigReliable()); err != nil {
		panic(err)
	}
	...
	kvServer := NewKVServerReliable(maxSize)
	kv.RegisterKVServiceServer(server, kvServer)

	logging.Info("Reliable KV server starting", zap.String("addr", ":11000"))
	server.Start()
}
