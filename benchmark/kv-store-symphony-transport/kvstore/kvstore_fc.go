...
func NewKVServerFC(maxSize int) *kvServerFC {
	if maxSize <= 0 {
...
func (s *kvServerFC) Get(ctx context.Context, req *kv.GetRequest) (*kv.GetResponse, context.Context, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *kvServerFC) Set(ctx context.Context, req *kv.SetRequest) (*kv.SetResponse, context.Context, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *kvServerFC) moveToEnd(key string) {
...
func main() {
	...
	if err := logging.Init(getLoggingConfigFC()); err != nil {
		panic(err)
	}
	...
	serverHandler := flowcontrol.NewFCServerHandler(
		udpTransport,
	...
	fcFeedbackChain := transport.NewHandlerChain("ServerFCFeedbackHandlerChain", serverHandler)
	...
	kvServer := NewKVServerFC(maxSize)
	kv.RegisterKVServiceServer(server, kvServer)

	logging.Info("Flow control KV server starting", zap.String("addr", ":11000"))
	server.Start()
}
