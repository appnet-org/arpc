#!/usr/bin/env python3
"""
Hotel Reservation Serialization Latency CDF Plotter

This script computes and plots Cumulative Distribution Functions (CDFs) of latency
for read and write operations across different serialization formats using the
Hotel Reservation benchmark payloads.
...
Output:
    - hotel_reservation_serialization_latency_cdf.pdf: A PDF file containing side-by-side CDF plots
      for write and read latencies with a shared legend at the bottom (base formats only).
    - hotel_reservation_serialization_latency_cdf_hybrid.pdf: A PDF file when --include-hybrid is used,
...
OUTPUT_FILE_BASE = "hotel_reservation_serialization_latency_cdf.pdf"
OUTPUT_FILE_HYBRID = "hotel_reservation_serialization_latency_cdf_hybrid.pdf"
...
def main():
    parser = argparse.ArgumentParser(description='Plot CDF of serialization latency for Hotel Reservation benchmark')
